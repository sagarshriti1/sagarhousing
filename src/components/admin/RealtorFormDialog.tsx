...
// Simulated Payment Form Component
const SimulatedPaymentForm = ({ paid, onPaymentComplete }: { paid: boolean; onPaymentComplete: () => void }) => {
  ...
  const isValid = cardNumber.replace(/\s/g, "").length === 16 && expiry.length === 5 && cvc.length >= 3 && cardName.length > 0;
  ...
  if (paid) {
    return (
      <div className="p-4 rounded-md border border-border bg-background text-center space-y-2">
        ...
        <p className="text-xs text-muted-foreground">Realtor listing fee has been paid</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-md border border-border bg-background p-4 space-y-4">
      ...
      >
        ...
        {processing ? "Processing..." : "Pay Rs. 5,000"}
      </Button>
