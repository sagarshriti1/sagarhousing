...
const RealtorFormDialog = ({
  ...
  const datesValid =
    ...
    new Date(form.start_date) < new Date(form.expiration_date);
  const bypassReasonValid =
    !bypassPayment ||
    realtorPromoFree ||
    !!(form.bypass_reason && form.bypass_reason.trim().length >= 3);
  const featuredDatesValid =
    !form.is_featured ||
    (!!form.featured_start_date &&
      !!form.featured_expiration_date &&
      new Date(form.featured_start_date) <
        new Date(form.featured_expiration_date));
  const featuredBypassReasonValid =
    !bypassFeatured ||
    featuredPromoFree ||
    !!(
  ...
  const handleSubmit = () => {
    ...
    if (
      ...
      errs.bypass_reason =
        'Please provide a reason (min 3 characters) for bypassing payment';
    }
    if (form.is_featured) {
      ...
      if (
        ...
        errs.featured_expiration_date =
          'Featured expiration must be after featured start';
      }
      if (
        bypassFeatured &&
        !featuredPromoFree &&
        ...
        errs.featured_bypass_reason =
          'Please provide a reason (min 3 characters) for bypassing featured payment';
      }
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='max-w-2xl max-h-[90vh] overflow-y-auto'>
        ...
        <div className='space-y-6'>
          ...
          >
            {/* Featured Subscription Collapsible */}
            <AccordionItem
              value='featured'
                ...
                <div className='rounded-lg border border-border p-4 space-y-4 bg-muted/30'>
                  <div className='flex items-center justify-between'>
                    <div>
                      ...
                      <p className='text-xs text-muted-foreground'>
                        Boosts this realtor in the directory for a one-month
                        term.
                      </p>
                    </div>
                    <Switch
                    ...
                    <>
                              ...
                              <Calendar
                                ...
                                onSelect={date => {
                                  ...
                                  setForm({
                                    ...
                                    featured_expiration_date: addMonths(s, 1),
                                  });
                                ...
                                disabled={
                                  form.featured_start_date
                                    ? {
                                        ...
                                        to: new Date(form.featured_start_date),
                                      }
                                    : undefined
                                }
                                initialFocus
                          ...
                          variant={
                            form.featured_payment_status === 'paid'
                              ...
                              : form.featured_payment_status === 'bypassed' ||
                                ...
                                ? 'secondary'
                                : form.featured_payment_status === 'expired'
                                  ? 'destructive'
                                  : 'secondary'
                          }
                        ...
                        <div className='flex items-center gap-3 p-3 rounded-md border border-accent/40 bg-accent/10'>
                          ...
                          <div className='flex-1 text-sm'>
                            <p className='font-medium text-foreground'>
                              🎉{' '}
                              {featuredPromoLabel ||
                                'Free featured promotion active'}
                            </p>
                            <p className='text-xs text-muted-foreground'>
                              No payment required for featured placement right
                              now.
                            </p>
                          </div>
                        </div>
                      ...
                      {form.featured_start_date &&
                        ...
                        new Date(form.featured_start_date) >=
                          new Date(form.featured_expiration_date) && (
                          <p className='text-xs text-destructive'>
                            Featured start date must be earlier than featured
                            expiration date.
                          </p>
                        )}
                    </>
            ...
            {/* Subscription & Payments Collapsible */}
            <AccordionItem
              value='subscription'
              className='border rounded-lg bg-card px-4'
            >
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        ...
                        >
                          <CalendarIcon className='mr-2 h-4 w-4' />
                          {form.expiration_date
                            ? format(new Date(form.expiration_date), 'PPP')
                            : 'Pick expiration date'}
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className='w-auto p-0' align='start'>
                ...
                <div className='rounded-lg border border-border p-4 space-y-4 bg-muted/30'>
                  <div className='flex items-center justify-between'>
                    <div>
                      ...
                      <p className='text-xs text-muted-foreground'>
                        {isCreate
                          ? 'Payment is required to activate realtor profile'
                  ...
                  {realtorPromoFree && (
                    <div className='flex items-center gap-3 p-3 rounded-md border border-accent/40 bg-accent/10'>
                      ...
                      <div className='flex-1 text-sm'>
                        ...
                        <p className='text-xs text-muted-foreground'>
                          No payment required for this service right now.
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {bypassPayment && !realtorPromoFree && (
                    <div className='space-y-2 p-3 rounded-md border border-amber-500/40 bg-amber-500/5'>
                      ...
                      {errors.bypass_reason ? (
                        ...
                        <p className='text-xs text-muted-foreground'>
                          This reason is mandatory and will be visible in the
                          payment history for both the admin and the realtor.
                        </p>
                      )}
                    </div>
                  )}
