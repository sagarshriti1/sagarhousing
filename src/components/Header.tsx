...
import { Home, Menu, X, Heart, User, Plus, LogOut } from "lucide-react";
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-border bg-card animate-fade-in">
          <nav className="container py-4 flex flex-col gap-3">
            ...
            {user ? (
              <>
                <Link to="/list-property" className="text-sm font-medium py-2 text-foreground">List Property</Link>
                <Link to="/my-listings" className="text-sm font-medium py-2 text-foreground">My Listings</Link>
                <Button size="sm" variant="outline" onClick={signOut} className="w-full mt-2">Sign Out</Button>
              </>
            ) : (
              <Link to="/auth">
