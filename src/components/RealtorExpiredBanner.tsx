...
const RealtorExpiredBanner = ({
  realtorId,
  onRenewed,
}: RealtorExpiredBannerProps) => {
  ...
  const {
    ...
    isFree,
    promoLabel,
  } = useFeatureFlag(FEATURE_KEYS.REALTOR_RENEWAL);

  const handlePaymentComplete = async () => {
    // UPDATED LOGIC TO ALWAYS REFRESH START DATE
    const now = new Date();
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        start_date: now.toISOString().split('T')[0], // Sets to today
    ...
    toast.success(
      isFree
        ...
        : 'Subscription renewed! Redirecting...',
    );
      ...
      <Card className='w-full max-w-md'>
        <CardHeader className='text-center'>
          ...
          <CardDescription>
            {isFree
              ? promoLabel ||
                '🎉 Free promotion active — renew your subscription at no cost.'
