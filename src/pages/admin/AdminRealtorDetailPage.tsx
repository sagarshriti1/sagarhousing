...
const AdminRealtorDetailPage = () => {
  ...
  const handleDelete = () => {
    setConfirm({
      ...
      description: `This will permanently delete "${realtor?.name}". This action cannot be undone.`,
      onConfirm: async () => {
  ...
  const handleSaveRealtor = async (data: RealtorFormData) => {
    ...
    const { data: flagRow } = await supabase
      .from('feature_flags')
      .select('*')
      .eq('key', FEATURE_KEYS.REALTOR_RENEWAL)
      .maybeSingle();
    ...
    const featuredChanged =
      data.is_featured &&
      (!realtor.is_featured ||
        (realtor.featured_expiration_date ?? null) !==
        ...
        <Card>
              ...
              <div className='flex items-center gap-4'>
                ...
                <div>
                  ...
                  <div className='flex items-center gap-2 mt-1 flex-wrap'>
                    <Badge variant={notExpired ? 'default' : 'secondary'}>
                      {notExpired ? 'Active' : 'Inactive'}
                    </Badge>
                    <Badge variant='outline' className='capitalize'>
                      Payment: {realtor.payment_status}
                    </Badge>
                  </div>
                </div>
              </div>
          ...
          <CardContent className='grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3 text-sm'>
            <div>
              ...
              <span className='text-foreground'>{realtor.email || '—'}</span>
            </div>
            <div>
              <span className='text-muted-foreground'>Phone:</span>{' '}
              <span className='text-foreground'>{realtor.phone || '—'}</span>
            </div>
            <div>
            ...
            <div>
              ...
              <span className='text-foreground'>
                {realtor.created_at
                  ? format(new Date(realtor.created_at), 'MMM d, yyyy')
                  : '—'}
              </span>
            </div>
            <div>
            ...
            <div>
              ...
              <span className='text-foreground'>
                ...
                {(() => {
                  ...
                  const d = safeParseDate(realtor.featured_expiration_date);
                  if (!d) return null;
          ...
          <CardContent>
            ...
            {properties.length === 0 ? (
              <p className='text-sm text-muted-foreground'>
                {realtor.email ? (
                  <>
                    No listings yet. When this realtor signs up with{' '}
                    <span className='font-medium text-foreground'>
                      {realtor.email}
                    </span>
                    , their listings will appear here automatically.
                  </>
                ) : (
